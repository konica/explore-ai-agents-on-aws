"""Hospital scheduling agent deployed on EKS (Fargate profile).
...
class OneLineJsonFormatter(logging.Formatter):
    ...
    Container log drivers (Fluent Bit on EKS Fargate, Docker's awslogs) forward
    stdout to CloudWatch one line at a time, so a multi-line traceback normally
    lands as a separate log event per line.
